...
class Module:
    ...
    def matmul(A: T.handle, B: T.handle, C_handle: T.handle):
        ...
        C_local = T.decl_buffer((128,), scope="local")
        ...
        with T.decl_buffer((3, 1, 16, 256), "float16", scope="shared.dyn") as A_shared:
            B_shared = T.decl_buffer((3, 2, 4, 512), "float16", scope="shared.dyn")
            ...
            for k in T.serial(30, annotations={"tl_pipelined_num_stages": 3}):
                with T.attr(0, "async_scope", 1):
                    ...
                    for i_1 in T.unroll(4):
                        for vec in T.vectorized(8):
                            T.ptx_cp_async(T.tvm_access_ptr(T.type_annotation("float16"), A_shared.data, (k + 2) % 3 * 4096 + i_1 * 1024 + thread_binding // 4 * 32 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 16 + (thread_binding % 16 // 8 + thread_binding % 2) % 2 * 8 + vec, 1, 2), T.tvm_access_ptr(T.type_annotation("float16"), A_1.data, by * 131072 + i_1 * 32768 + thread_binding // 4 * 1024 + k * 32 + thread_binding % 4 * 8 + vec + 64, 1, 1), 1)
                with T.attr(0, "async_scope", 1):
                    T.attr(0, "tl.pipeline_mvb_stage_expr", (k + 2) % 3)
                    T.attr(0, "tl.pipeline_mvb_parity_expr", (k + 2) % 6 // 3)
                    i_1 = T.int32()
                ...
                for ki in range(2):
                    A_local = T.decl_buffer((32,), "float16", scope="local")
                    B_local = T.decl_buffer((32,), "float16", scope="local")
                    for i_1 in range(4):
                        T.ptx_ldmatrix(T.bool(False), 4, T.tvm_access_ptr(T.type_annotation("float16"), A_shared.data, k % 3 * 4096 + thread_binding % 64 // 32 * 2048 + i_1 * 512 + (thread_binding % 16 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16) // 256 * 256 + thread_binding % 8 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 8, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), A_local.data, i_1 * 8, 8, 2))
                    for i_1 in range(4):
                        T.ptx_ldmatrix(T.bool(True), 4, T.tvm_access_ptr(T.type_annotation("float16"), B_shared.data, k % 3 * 4096 + thread_binding // 64 * 2048 + ki * 1024 + (thread_binding % 16 * 64 + ((i_1 * 16 + thread_binding % 32 // 16 * 8) // 32 + thread_binding % 8 // 4) % 2 * 32) // 512 * 512 + (thread_binding % 16 * 64 + ((i_1 * 16 + thread_binding % 32 // 16 * 8) // 32 + thread_binding % 8 // 4) % 2 * 32 + (thread_binding % 4 // 2 + i_1 % 2) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 2) % 2 * 8) % 512, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), B_local.data, i_1 * 8, 8, 2))
                    for i_1, j in T.grid(4, 4):
                        T.ptx_mma("float32", "m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_local.data, i_1 * 8, B_local.data, j * 8, C_local.data, i_1 * 32 + j * 8, T.bool(False))
                        T.ptx_mma("float32", "m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_local.data, i_1 * 8, B_local.data, j * 8 + 4, C_local.data, i_1 * 32 + j * 8 + 4, T.bool(False))
            T.ptx_wait_group(2)
            with T.attr(0, "tl.pipeline_mvb_stage_expr", 0):
                ...
                for ki in range(2):
                    ...
                    B_local = T.decl_buffer((32,), "float16", scope="local")
                    for i_1 in range(4):
                        T.ptx_ldmatrix(T.bool(False), 4, T.tvm_access_ptr(T.type_annotation("float16"), A_shared.data, thread_binding % 64 // 32 * 2048 + i_1 * 512 + (thread_binding % 16 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16) // 256 * 256 + thread_binding % 8 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 8, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), A_local.data, i_1 * 8, 8, 2))
                    for i_1 in range(4):
            ...
            for ki in range(2):
                ...
                B_local = T.decl_buffer((32,), "float16", scope="local")
                for i_1 in range(4):
                    T.ptx_ldmatrix(T.bool(False), 4, T.tvm_access_ptr(T.type_annotation("float16"), A_shared.data, thread_binding % 64 // 32 * 2048 + i_1 * 512 + (thread_binding % 16 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16) // 256 * 256 + thread_binding % 8 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 8 + 4096, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), A_local.data, i_1 * 8, 8, 2))
                for i_1 in range(4):
                    T.ptx_ldmatrix(T.bool(True), 4, T.tvm_access_ptr(T.type_annotation("float16"), B_shared.data, thread_binding // 64 * 2048 + ki * 1024 + (thread_binding % 16 * 64 + ((i_1 * 16 + thread_binding % 32 // 16 * 8) // 32 + thread_binding % 8 // 4) % 2 * 32) // 512 * 512 + (thread_binding % 16 * 64 + ((i_1 * 16 + thread_binding % 32 // 16 * 8) // 32 + thread_binding % 8 // 4) % 2 * 32 + (thread_binding % 4 // 2 + i_1 % 2) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 2) % 2 * 8) % 512 + 4096, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), B_local.data, i_1 * 8, 8, 2))
                for i_1, j in T.grid(4, 4):
                    T.ptx_mma("float32", "m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_local.data, i_1 * 8, B_local.data, j * 8, C_local.data, i_1 * 32 + j * 8, T.bool(False))
                    T.ptx_mma("float32", "m16n8k16", "row", "col", "fp16", "fp16", "fp32", A_local.data, i_1 * 8, B_local.data, j * 8 + 4, C_local.data, i_1 * 32 + j * 8 + 4, T.bool(False))
        i_1 = T.int32()
        ...
        for i_1 in T.unroll(64):
            ...
            for vec in T.vectorized(2):
                C_local_cast[vec] = T.Cast("float16", C_local[i_1 * 2 + vec])
            for vec_copy in T.vectorized(2):
                C[by * 128 + thread_binding % 64 // 32 * 64 + i_1 // 16 * 16 + i_1 % 2 * 8 + thread_binding % 32 // 4, bx * 128 + thread_binding // 64 * 64 + i_1 % 16 // 2 * 8 + thread_binding % 4 * 2 + vec_copy] = C_local_cast[vec_copy]