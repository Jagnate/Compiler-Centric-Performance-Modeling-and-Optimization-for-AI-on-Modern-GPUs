...
class Module:
    ...
    def matmul(A: T.handle, B: T.handle, C_handle: T.handle):
        T.func_attr({"target": T.target({"arch": "sm_86", "host": {"keys": ["cpu"], "kind": "c", "tag": ""}, "keys": ["cuda", "gpu"], "kind": "cuda", "max_num_threads": 1024, "tag": "", "thread_warp_size": 32}), "tilelang_out_idx": [-1], "tir.is_entry_func": True, "tl.has_tma": T.bool(False), "tl.readonly_param_indices": [0, 1, 2], "tma_descriptor_args": {}})
        ...
        Module.matmul_kernel(A_1.data, B_1.data, C.data)
    ...
    def matmul_kernel(A: T.handle("float16", "global"), B: T.handle("float16", "global"), C: T.handle("float16", "global")):
        ...
        with T.attr(0, "tl.pipeline_mvb_num_stages", 3):
            with T.attr(0, "async_scope", 1):
                ...
                for i in T.unroll(4):
                    T.ptx_cp_async(T.tvm_access_ptr(T.type_annotation("float16"), A_shared, i * 1024 + thread_binding // 4 * 32 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 16 + (thread_binding % 16 // 8 + thread_binding % 2) % 2 * 8, 1, 2), T.tvm_access_ptr(T.type_annotation("float16"), A, by * 131072 + i * 32768 + thread_binding // 4 * 1024 + thread_binding % 4 * 8, 1, 1), 8)
            with T.attr(0, "async_scope", 1):
                T.attr(0, "tl.pipeline_mvb_stage_expr", 0)
                T.attr(0, "tl.pipeline_mvb_parity_expr", 0)
                for i in T.unroll(4):
