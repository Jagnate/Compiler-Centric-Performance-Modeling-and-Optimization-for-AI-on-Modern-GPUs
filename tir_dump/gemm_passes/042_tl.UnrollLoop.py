    ...
    def matmul(A: T.handle, B: T.handle, C_handle: T.handle):
        ...
        with T.attr(0, "tl.pipeline_mvb_num_stages", 3):
            ...
            for k in T.serial(30, annotations={"tl_pipelined_num_stages": 3}):
                ...
                for ki in range(2):
                    for i in range(4):
                        T.ptx_ldmatrix(T.bool(False), 4, T.tvm_access_ptr(T.type_annotation("float16"), A_shared, k % 3 * 4096 + thread_binding % 64 // 32 * 2048 + i * 512 + (thread_binding % 16 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16) // 256 * 256 + thread_binding % 8 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 8, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), A_local, i * 8, 8, 2))
                    for i in range(4):
            ...
            for ki in range(2):
                for i in range(4):
                    T.ptx_ldmatrix(T.bool(False), 4, T.tvm_access_ptr(T.type_annotation("float16"), A_shared, thread_binding % 64 // 32 * 2048 + i * 512 + (thread_binding % 16 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16) // 256 * 256 + thread_binding % 8 * 32 + (thread_binding % 8 // 4 + ki) % 2 * 16 + (thread_binding % 32 // 16 + thread_binding % 4 // 2) % 2 * 8 + 4096, 8, 1), T.tvm_access_ptr(T.type_annotation("float16"), A_local, i * 8, 8, 2))
                for i in range(4):
