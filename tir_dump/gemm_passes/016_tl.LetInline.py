    ...
    def _gemm_ssr():
        ...
        for ki in range(2):
            ...
            for i in range(4):
                ...
                T.ptx_ldmatrix(T.bool(False), 4, T.access_ptr(A_shared[0, thread_binding // 32 % 2 * 64 + i * 16 + thread_binding % 32 % 16, ki * 16 + 8 * (thread_binding % 32 // 16)], 8, 1), T.access_ptr(A_local[i * 8], 8, 2))
            for i in range(4):
                B_shared = T.Buffer((3, 32, 128), "float16", scope="shared.dyn")
                T.ptx_ldmatrix(T.bool(True), 4, T.access_ptr(B_shared[0, ki * 16 + thread_binding % 32 % 16, thread_binding // 64 % 2 * 64 + i * 16 + 8 * (thread_binding % 32 // 16)], 8, 1), T.access_ptr(B_local[i * 8], 8, 2))
